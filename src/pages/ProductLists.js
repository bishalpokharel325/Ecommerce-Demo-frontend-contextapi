...
const ProductLists = () => {
    ...
    },[brand,cod,view,keyword,freeship,price_min,price_max,ratingmin,ratingmax,color,warranty,sortfield,discount,sortdirection])
    return (
                ...
                <div className="app__body__right">
                    <div className="app__right__top">
                        <div className="right__header">
                            <h3>Mobiles/Samsung</h3>
                        </div>
                        <div className="right__second__header">
                            <p><i>Found {productdata.length} Items</i></p>
                    ...
                    <div className="app__right__bottom">
                      ...
                      {productdata.map((item)=>{
                         ...
                         return (<Link to={`/productdetails/${item.slug}`}><ProductCardWide key={item.id} description={item.description} title={item.title} price={item.price} rating={item.rating} src={`http://localhost:8000${item.image}`}/></Link>)
                           ...
                           return <Link to={`/productdetails/${item.slug}`}><ProductCard key={item.id} title={item.title} price={item.price} rating={item.rating} src={`http://localhost:8000${item.image}`}/></Link> 
                       })
                     }
                      </div>
...
export {categoryContext,discountContext,brandContext,codContext,freeshipContext,priceminContext,pricemaxContext,ratingminContext,ratingmaxContext,colorContext,warrantyContext,sortdirectionContext,sortfieldContext}
